...
const STATUS_LABEL = { new: 'नया', read: 'पढ़ लिया', replied: 'जवाब दिया' };
...
export default function AdminMessages() {
  ...
  const remove = async (msg) => {
    if (!window.confirm('यह संदेश हटाना है?')) return;
  ...
  return (
    <div>
      <header className="mb-6">
        <h1 className="text-2xl text-wood-900">संदेश / Messages</h1>
        <p className="mt-1 text-sm text-muted">Contact फॉर्म से आए सारे संदेश</p>
      </header>
      ...
      {!error &&
        (loading ? (
          ...
          <div className="card p-10 text-center text-sm text-muted">अभी कोई संदेश नहीं आया</div>
        ) : (
