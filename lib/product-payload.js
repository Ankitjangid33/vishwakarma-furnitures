...
export function buildTestimonialPayload(body = {}) {
  const stars = Math.round(Number(body.stars) || 5);
  return {
    name: sanitize(body.name, 80),
    place: { en: sanitize(body?.place?.en, 80), hi: sanitize(body?.place?.hi, 80) },
    text: { en: sanitize(body?.text?.en, 600), hi: sanitize(body?.text?.hi, 600) },
